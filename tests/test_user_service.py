...
def test_create_user_creates_user_and_profile_together(db_session):
    user, profile = user_service.create_user(db_session, _make_payload())

    assert user.id is not None
    assert profile is not None
    assert profile.user_id == user.id
    assert profile.expected_daily_hours == 8.0
    assert profile.remaining_vacation_days == 21.0


def test_create_user_hashes_password(db_session):
    user, _profile = user_service.create_user(db_session, _make_payload(password="supersecret123"))
...
def test_get_profile_returns_profile(db_session):
    created, _profile = user_service.create_user(db_session, _make_payload())

    profile = user_service.get_profile(db_session, created.id)

    assert profile.user_id == created.id


def test_get_profile_raises_when_not_found(db_session):
    with pytest.raises(ValueError):
        user_service.get_profile(db_session, 999)


def test_update_profile_applies_partial_fields(db_session):
    created, _profile = user_service.create_user(db_session, _make_payload())
...
def test_update_profile_raises_when_not_found(db_session):
    with pytest.raises(ValueError):
        user_service.update_profile(db_session, 999, EmployeeProfileUpdate())
...
def test_get_user_with_profile_returns_user_with_profile(db_session):
    created, _profile = user_service.create_user(db_session, _make_payload())

    user, profile = user_service.get_user_with_profile(db_session, created.id)

    assert user.id == created.id
    assert profile is not None


def test_get_user_with_profile_raises_when_not_found(db_session):
    with pytest.raises(ValueError):
        user_service.get_user_with_profile(db_session, 999)
